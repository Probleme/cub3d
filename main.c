...
/*   Updated: 2023/10/12 23:36:37 by ataouaf          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "inc/cub3d.h"

void ft_rotation_angle(t_cube *cube, char direction)
{
	if (direction == 'R')
		...
		while (cube->parse->map2d->map[(int)pos.y][(int)++pos.x])
		{
			if (is_player(cube->parse->map2d->map[(int)pos.y][(int)pos.x]))
			{
				...
				if (direction == 'N')
					...
					exit(ft_dprintf(2,
							"Failed to set initial player rotation\n"));
...
void	ft_init_images(t_cube *cube)
{
	...
	if (mlx_image_to_window(cube->mlx.mlx, cube->mlx.img->floor, 0, HEIGHT
			/ 2) == -1)
	...
	if (mlx_image_to_window(cube->mlx.mlx, cube->mlx.img->mini_map, 30, 20) ==
		-1)
